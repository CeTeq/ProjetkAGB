...
} = await useFetch("http://" + devIP + ":3003/api/getProjects", {
    method: "GET",
    headers: {
...
async function addProject() {
    ...
    try {
        const response = await $fetch(
            "http://" + devIP + ":3003/api/createProject",
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
        ...
        navigateTo(
            `/project/${projectsData.value[projectsData.value.length - 1].id}`,
        );
    } catch (error) {
        console.log(error);
        toast.add({
...
function validate() {
    ...
    if (!state.projectName)
        errors.push({ name: "projectName", message: "Required" });
    return errors;
}
...
<template>
    <UApp>
        ...
        <div v-else class="fixed inset-0 flex flex-col overflow-hidden">
            <header
                class="flex items-center gap-4 px-6 py-4 border-b border-default shrink-0 bg-background"
            >
                <div class="flex-1">
                    <h1 class="text-lg font-semibold">
                        {{
                            items.find((i) => i.slot === activeDesktopTab)
                                ?.label
                        }}
                    </h1>
            ...
            <nav
                class="flex gap-1 px-6 py-2 border-b border-default shrink-0 bg-background"
            >
                <button
                    v-for="tab in items"
                    :key="tab.slot"
        ...
        <UModal
            color="neutral"
            variant="subtle"
            v-model:open="addProjectModalState"
            class="p-4 w-[60%] divide-y-0"
        >
            <template #content class="flex justify-center">
                <div class="pb-3 justify-center pb-2 w-full text-center">
                ...
                >
                    ...
                    <div class="flex justify-center">
                        <UButton
                            type="submit"
                            class="w-15 ml-0 mp-3 justify-center"
                            color="success"
                            loading-auto
