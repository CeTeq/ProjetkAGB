...
} = await useFetch("http://" + devIP + ":3003/api/project", {
    method: "GET",
    headers: {
...
const projectDataComputed = computed(() => {
    ...
    return {
        client: {
            id: projectData.value.client.id ?? null,
            name: projectData.value.client.name ?? null,
            street: projectData.value.client.street ?? null,
            street_number: projectData.value.client.street_number ?? null,
            post_code: projectData.value.client.post_code ?? null,
            city: projectData.value.client.city ?? null,
            date: projectData.value.client.date ?? null,
            description: projectData.value.client.description ?? null,
        },
    };
});
const clientDataRows = computed(() => {
    if (!projectDataComputed.value?.client) return [];

    const client = projectDataComputed.value.client;
    return [
        { label: "ID", value: client.id || "—" },
        { label: "Nazwa", value: client.name || "—" },
        { label: "Ulica", value: client.street || "—" },
        { label: "Nr domu", value: client.street_number || "—" },
        { label: "Kod pocztowy", value: client.post_code || "—" },
        { label: "Miasto", value: client.city || "—" },
        { label: "Data", value: client.date || "—" },
        { label: "Opis", value: client.description || "—" },
    ];
});
...
const { data: allItemsData, errorAllItems } = await useFetch(
    `http://${devIP}:3003/api/getProducts`,
    {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
...
<template>
    <ClientOnly>
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <UDropdownMenu
                                            ...
                                            "
                                            class="flex absolute right-4 sm:r-6"
                                        >
                                            <UButton
                                                icon="lucide:more-vertical"
                                                color="neutral"
                                                variant="ghost"
                                                class="w-fit ml-auto z-10 hover:bg-[#FFFFFF1F]"
                                            />
                                        </UDropdownMenu>
                                    </div>
                                </div>
                                <div
                                    class="fixed left-[85%] top-[92%] z-100 md:hidden print:hidden"
                                >
                                    <UButton
                                        icon="lucide:plus"
                                        class="p-4"
        ...
        <div v-else class="fixed inset-0 flex overflow-hidden print:hidden">
            <aside
                class="w-56 shrink-0 flex flex-col border-r border-default bg-background"
            >
                <div class="px-4 py-5 border-b border-default">
                    <div
                        class="text-xs font-semibold uppercase tracking-widest text-muted mb-1"
                    >
                        Projekt
                    </div>
            ...
            <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
                <header
                    class="flex items-center gap-4 px-6 py-4 border-b border-default shrink-0 sticky top-0 z-10 bg-background"
                >
                    <div class="flex-1">
                        <h1 class="text-lg font-semibold">
                            {{
                                tabItems.find(
                                    (t) => t.slot === activeDesktopTab,
                                )?.label
                            }}
                        </h1>
                        <p
                ...
                <div class="flex-1 overflow-y-auto">
                    <template v-if="activeDesktopTab === 'items'">
                        ...
                        >
                            ...
                            />
                            <p class="text-sm">
                                Brak przedmiotów w tym projekcie.
                            </p>
                            <UButton
                                variant="outline"
                                size="sm"
                                @click="addItemModalState = true"
                                >Dodaj pierwszy przedmiot</UButton
                            >
                        </div>
                        ...
                        <table v-else class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-default text-left">
                                    <th
                                        class="px-6 py-3 text-xs font-semibold uppercase tracking-wide text-muted w-10"
                                    >
                                        #
                                    </th>
                                    <th
                                        class="px-6 py-3 text-xs font-semibold uppercase tracking-wide text-muted"
                                    >
                                        Nazwa
                                    </th>
                                </tr>
                            ...
                            <tbody>
                                <tr
                                    v-for="(item, index) in projectItems.filter(
                                        (i) =>
                                            !searchTerm ||
                                            i.label
                                                .toLowerCase()
                                ...
                                >
                                    <td
                                        class="px-6 py-4 text-muted tabular-nums"
                                    >
                                        {{ index + 1 }}
                                    </td>
                                    <td class="px-6 py-4">
        ...
        <UModal
            color="neutral"
            variant="subtle"
            v-model:open="addItemModalState"
            class="p-4 w-[80%] divide-y-0 print:hidden"
            @submit.prevent="addProject"
        >
            <template #content class="flex justify-center">
                <div class="pb-3 justify-center w-full text-center">
                ...
                <UForm>
                    <div class="text-center">
                        <UFormField name="projectName">
                            <UListbox
                                :items="allItems"
                                v-model:search-term="searchTermModal"
                                filter
                            >
                                <template #item="{ item }">
                                    <div
        ...
        <UModal
            color="neutral"
            variant="subtle"
            v-model:open="confirmModalState"
            class="p-4 w-[80%] print:hidden"
        >
            <template #content class="flex justify-center">
                <div class="mb-5 justify-center pb-2">
                    Czy na pewno chcesz usunąć przedmiot o nazwie: "{{
                        confirmoModalItem
                    }}"
                </div>
                <div class="flex justify-center">
                    <UButton
                        class="w-15 ml-3 mp-3 justify-center"
                        color="success"
                        @click="removeItem(tempModalItemID)"
                        loading-auto
                        :dismissible="false"
                        non-dismissible
                        >Tak</UButton
    ...
    <UFooter class="print:hidden"> </UFooter>
</template>
