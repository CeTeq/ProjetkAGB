        ...
        getNextId('Shrack_cennik', 'Id_cennika')
            .then(result => {
                ...
                db.serialize(async ()=>{
                    ...
                    const stmt = db.prepare("INSERT INTO Shrack_cennik (Id_cennika, Data_Dodania) VALUES (?,?)")
                    const products = db.prepare("INSERT INTO Produkty (Id_produktu, Nazwa_Produktu, Znacznik_Produktu, Cena_Produktu, Waluta, Id_cennika) VALUES (?,?,?,?,?,?)")
                    try {
                        const dateNow = new Date()
                        let date = [dateNow.getDate(), dateNow.getMonth(), dateNow.getFullYear(), dateNow.getHours(), dateNow.getMinutes(), dateNow.getSeconds()];
                        date.forEach((d, i) => {
                            if (d < 10) date[i] = '0' + d
                        })

                        stmt.run(cennikID, date[0] + '-' + date[1] + '-' + date[2] + ' ' + date[3] + ':' + date[4] + ':' + date[5]);
                        ...
                        cennik.forEach((element, i) => {
                            if(i !== 0){
                                products.run(iDProduktu + i - 1, element[0], element[1], element[2], 'PLN', cennikID);
                            }
                        })
...
function validateInput(input) {
    ...
    input.forEach((element, i)=>{
        if(correct === true) { element.forEach((column, j)=>{
