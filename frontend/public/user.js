...
async function getData() {
	...
	try {
		...
		json.forEach((element) => {
			...
			project.querySelector(".description").innerText =
				element.description || "Brak opisu";
...
function newProject() {
	...
	document.querySelector("#projectName").addEventListener("input", (e) => {
		if (document.querySelector("#projectName").value) {
			...
			document.getElementById("createProject").setAttribute("disabled", "");
	});
}
...
function archiveProject(card) {
	...
	document.body.appendChild(dialog);
	dialog.innerHTML =
		"<div>Czy na pewno chcesz zarchiwizować projekt " +
		"<b>" +
		'"' +
		cardName +
		'"' +
...
function unarchiveProject(card) {
	...
	document.body.appendChild(dialog);
	dialog.innerHTML =
		"<div>Czy chcesz przywrócić projekt " +
		"<b>" +
		'"' +
		cardName +
		'"' +
