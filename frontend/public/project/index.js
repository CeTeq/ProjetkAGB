...
async function getData() {
	...
	try {
		...
		const goBackButtonCenter = document.createElement("mdui-button-icon");
		goBackButtonCenter.className = "goBackButtonCenter";
		goBackButtonCenter.setAttribute('disabled', '')
		...
		topBar.appendChild(goBackButtonCenter);
...
async function projectData(client) {
	...
	table.innerHTML = `
    <tr id="city"><th>Nazwa klienta:</th><td><input class="client-data-input" value="${client.name || "Brak"}"></td></tr>
    <tr><th>Miasto:</th><td id="clientCity"><input class="client-data-input" value ="${client.city || "Brak"}"</td></tr>
    <tr><th>Ulica:</th><td id="clientStreet"><input class="client-data-input" value ="${client.street || "Brak"}" ${client.street_number || ""}"></td></tr>
    <tr><th>Opis:</th><td id="clientDesc"><input class="client-data-input" value ="${client.description || "Brak"}"</td></tr>
    <tr><th>Data stworzenia:</th><td id="id=""clientDate"><input class="client-data-input" value ="${client.date}"></td></tr>`;
...
const pendingChangesItems = new Map();
const ucSnakcBarItems = document.createElement("mdui-snackbar");
ucSnakcBarItems.setAttribute("action", "Zapisz");

ucSnakcBarItems.setAttribute("auto-close-delay", "0");
ucSnakcBarItems.setAttribute("close-on-outside-click", "false");
ucSnakcBarItems.innerText = "Masz niezapisane zmiany w produktach";
ucSnakcBarItems.addEventListener("action-click", async () => {
	if (pendingChangesItems.size > 0) {
		ucSnakcBarItems.open = false;
		await uploadItemChangesItems();
	}
});
document.querySelector("#itemsTab").appendChild(ucSnakcBarItems);

const pendingChangesClient = new Map();
const ucSnakcBarItemsClient = document.createElement("mdui-snackbar");
ucSnakcBarItemsClient.setAttribute("action", "Zapisz");

ucSnakcBarItemsClient.setAttribute("auto-close-delay", "0");
ucSnakcBarItemsClient.setAttribute("close-on-outside-click", "false");
ucSnakcBarItemsClient.innerText = "Masz niezapisane zmiany w produktach";
ucSnakcBarItemsClient.addEventListener("action-click", async () => {
	if (pendingChangesItems.size > 0) {
		ucSnakcBarItemsClient.open = false;
		await uploadItemChangesItems();
	}
});
document.body.appendChild(ucSnakcBarItems);

async function updateItem(item, amount) {
	if (item && amount) {
		pendingChangesItems.set(item.id, item);
		if (item.number == amount && pendingChangesItems.has(item.id)) {
			pendingChangesItems.delete(item.id);
			if (pendingChangesItems.size === 0) {
				ucSnakcBarItems.open = false;
			}
		} else {
			pendingChangesItems.set(item.id, amount);
			ucSnakcBarItems.open = true;
			unsavedChanges = true;
		}
	}
}
async function uploadItemChangesItems() {
	const items = [];
	pendingChangesItems.forEach((value, key) => {
...
async function editProjectInformation() {
	if (pendingChangesItems.size > 0) await uploadItemChangesItems();
	if ((await checkPermission()) <= 0) return;
	if (pendingChangesItems.size > 0) return;
